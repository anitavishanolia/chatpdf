    ...
    let newPromise = new Promise((resolve, reject) => {
      try {
        const documentRef = doc(
          db,
          `${user?.email}/files/pdf/${routeId}/members/${id}`
        );
...
function Share({ user, pdfDocument, routeId }: any) {
  ...
  const q = query(
    collection(db, `${user?.email}/files/pdf/${routeId}/members`)
  );
  ...
  const onAddMember = async () => {
    ...
    const docRef = collection(
      db,
      `${user?.email}/files/pdf/${routeId}/members`
    );
    let newPromise = new Promise(async (resolve, reject) => {
      try {
        const shareRef = collection(db, `share/pdf/${routeId}`);
        if (!creatorDetails) {
            ...
            <>
              ...
              <ModalBody>
                {!addMember ? (
                  ...
                  <>
                    <Snippet>{baseURL + "/share/pdf/" + routeId}</Snippet>
                  </>
                )}
                <div>
                  <MemberTable
